...
public abstract class NukeAbs extends Weapon {
    ...
    public NukeAbs(GameModel gameModel, String name, int level,int id,BufferedImage icon) {
        super(gameModel,name,id,Color.YELLOW);
        ...
        if(hit) {
            if (explosionTimer == 0) {
                ...
                if(explosionTimer < 30) {
                    ...
                    g2d.setColor(new Color(255, 255, 255, (int) (255 - 255 * explosionTimer / 30.0)));
                    g2d.fillRect(0,0, GameLoop.imgW,GameLoop.imgH);
                }
    ...
    public Weapon getLevelWeapon(int level,GameModel model){
        switch (level){
            case 1:return new Nuke(model);
            case 2:return new MegaNuke(model);
            default:return null;
        }
    }
